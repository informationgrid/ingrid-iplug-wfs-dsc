...
if (wfsRecord instanceof WFSFeature) {
	...
	// add the title
	addOutput(idfBody, "h1", getFeatureTitle(recordNode));
	
	//add the summary
	addOutput(idfBody, "p", getFeatureSummary(recordNode));
...
//
	...
	// add the title
	addOutput(idfBody, "h1", getFeatureTypeTitle(recordNode));
	
	//add the summary
	addOutput(idfBody, "p", getFeatureTypeSummary(recordNode));
	
	// add number of features
	addOutput(idfBody, "p", wfsRecord.getNumberOfFeatures()+" Feature(s)");
...
function getFeatureTypeSummary(recordNode) {
